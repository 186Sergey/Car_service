...
conn = sql.connect("carservise.db")
cur = conn.cursor()
...
def records():
    ...
    cur.execute('''INSERT INTO service(firma, region, city, street, myindex, 
                                       surname, username, patronymic,
                                       phfone, fax, email) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                   (firma, region, city, street, myindex, surname, username, patronymic,
                    phfone, fax, email))
    conn.commit()
...
label = tk.Label(topframe, text="Сведения о реквизитах СТО и собственнике", font=("Times New Roman", 14), fg="green")
label.grid(row=0, column=0, columnspan=6, padx=20, pady=20)

'''Первый ряд'''

service_detal = tk.LabelFrame(topframe, text="Сведения о собственнике и реквизитах СТО", fg="green", font=("Times New Roman", 12))
service_detal.grid(row=1, column=0, columnspan=6, padx=20, pady=20)
...
btn_send = tk.Button(topframe, text="Сохранить", width=25, bg="lightblue", command=records)
