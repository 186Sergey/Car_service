...
def save_users():
    """
    The function adds user data to the database. Checks the password for complexity (number input characters.)\n
    Функция добавляет данные о пользователях в БД. Проверяет пароль на сложность (количество вводимых символов.)
    """
    ...
    if len(passwd) < 5:
        ...
        cur.execute('''UPDATE users SET surname=?, username=?, password=? WHERE id=?''', (surname, username, passwd, '#1'))
    conn.commit()
    conn.close()
    root.destroy()                 
...
# def validate(input):
#     """
#     The function checks the Entry field for characters entered by the user from the keyboard, letters must be entered.\n
#     Функция проверяет поле Entry на вводимые символы пользователем с клавиатуры, должны быть введены буквы.
#     """
...
lbl_new_user = ttk.Label(root, text="* Пароль должен состоять из строчных и заглавных букв, цифр и символов", foreground="blue")
